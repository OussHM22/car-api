...
exports.createCar = (req, res) => {
    ...
    if (!brand || !model || !year) {
        return res.status(400).json({
            ...
            message: 'brand, model et year sont obligatoires'
        });
    }
    ...
    db.run(query, [brand, model, year, color, price, mileage, description], function (err) {
        if (err) {
            return res.status(500).json({
...
exports.updateCar = (req, res) => {
    ...
    db.get('SELECT * FROM cars WHERE id = ?', [id], (err, row) => {
        ...
        const query = `
            UPDATE cars SET brand=?, model=?, year=?, color=?, price=?, mileage=?, description=?
            WHERE id=?
        `;

        db.run(query, [brand, model, year, color, price, mileage, description, id], function (err) {
            if (err) {
                return res.status(500).json({
